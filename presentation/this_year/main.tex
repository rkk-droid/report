...
\begin{frame}
  ...
  \begin{columns}
    ...
    \vspace{0.1cm}
    {\small При $N_\text{neg}=1$: MRR $\in \{0.5, 1.0\}$,\\интерпретируется как доля правильно ранжированных пар}
  \end{columns}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item \textbf{PyTorch Geometric Temporal} — ориентирован на дискретные временные снимки; ограниченная поддержка темпоральности «из коробки»
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \begin{itemize}
      ...
      \item Автоматическое тестирование (47 тестовых файлов)
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \end{itemize}
    \column{0.47\textwidth}
    \textbf{Temporal GNN-архитектуры}
    \begin{itemize}
      ...
      \item \textbf{GLFormer}: адаптивная агрегация токенов с расширяющимся рецептивным полем; $O(N\sum K_l)$
      \item \textbf{DyGFormer}: трансформер над историческими последовательностями первых соседей; NCoE-кодирование
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item \textbf{Глубокие модели}: выбраны 4 архитектуры, покрывающие широкий спектр:
      \begin{itemize}
...
\begin{frame}
  ...
  \begin{columns}
    \column{0.55\textwidth}
    Каждый узел $v$ характеризуется \textbf{15 признаками}, вычисленными исключительно на обучающей выборке (без утечки данных):
    \begin{itemize}
      \item \textbf{Степенные}: $\log\,\text{in\_degree}$, $\log\,\text{out\_degree}$, \text{in\_out\_ratio}
      \item \textbf{Уникальные контрагенты}: $\log\,\text{unique\_in\_cp}$, $\log\,\text{unique\_out\_cp}$
      \item \textbf{Транзакционные объёмы}: $\log\,\text{total\_btc\_in/out}$, $\log\,\text{avg\_btc\_in/out}$
      \item \textbf{Временные паттерны}: recency, activity\_span, $\log\,\text{event\_rate}$, burstiness
      \item \textbf{Энтропия контрагентов}: out/in counterparty entropy
    \end{itemize}
    \column{0.42\textwidth}
    \textbf{Признаки пары $(s, d)$}:
    \begin{itemize}
      \item Конкатенация 15 признаков узла-источника $s$ и 15 признаков узла-назначения $d$
      \item Итого: вектор размерности \textbf{30}
    \end{itemize}
    \vspace{0.3cm}
    \textbf{Стратегия семплирования}: случайное семплирование негативных рёбер с фиксированным источником ($N_\text{neg} = 1$)
    \vspace{0.3cm}
    {\small Такая схема сводит ранжирование к бинарной классификации пар}
  \end{columns}
\end{frame}
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \textbf{11 пакетных областей канонического API:}
    \begin{itemize}
      \item \texttt{config} — типизированные YAML-конфигурации (\texttt{ExperimentSpec})
      \item \texttt{models} — адаптеры и реестр 7 семейств моделей
      \item \texttt{training} / \texttt{evaluation} — единый цикл обучения и оценки
      \item \texttt{data} — stream graph в parquet-формате
      \item \texttt{sampling} — стратегии негативного семплирования
      \item \texttt{cuda} — GPU-ускоренные примитивы
      \item \texttt{graph\_metrics}, \texttt{analysis}, \texttt{experiments}, \texttt{infra}
    \end{itemize}
    \column{0.45\textwidth}
    \textbf{Пользовательские сценарии:}
    \begin{enumerate}
      \item \textbf{YAML-запуск}: декларативное описание эксперимента
      \item \textbf{Python API}: прямое использование из кода
      \item \textbf{ИИ-ассистент}: встроенный агентный сценарий
    \end{enumerate}
    \vspace{0.3cm}
    \textbf{Количественные характеристики:}
    \begin{itemize}
      \item 7 моделей в реестре
      \item 6 источников данных
      \item 3 backend'а: Python / C++ / CUDA
      \item \textbf{47 тестовых файлов}
    \end{itemize}
  \end{columns}
\end{frame}
...
result = launch_experiment(spec)
    ...
    \vspace{0.2cm}
    GitHub: \texttt{paymentgraphforecasting/}\\
  ...
  \begin{columns}
    ...
    \begin{enumerate}
      \item \textbf{Поиск временных соседей} (\texttt{TemporalGraphSampler})\\
        ...
        {\small CUDA: CSR-граф переносится в GPU один раз, поиск и featurize выполняются на устройстве}
      \vspace{0.2cm}
      \item \textbf{Common Neighbors} (\texttt{CommonNeighbors})\\
        $\text{CN}(u, v) = |\mathcal{N}(u) \cap \mathcal{N}(v)|$\\
        {\small CUDA: bitset-представление множеств соседей, popcount-пересечение}
    \end{enumerate}
    \column{0.42\textwidth}
    \textbf{Бенчмарк: поиск временных соседей}
    \begin{table}
    \tiny
    \begin{tabular}{llr}
      \toprule
      Сценарий & Backend & Ускорение \\
      \midrule
      graphmixer\_small & Python & 1.0x \\
      graphmixer\_small & cpp & 35.9x \\
      graphmixer\_small & cuda & \textbf{49.9x} \\
      \midrule
      dygformer\_medium & Python & 1.0x \\
      dygformer\_medium & cpp & 1.8x \\
      dygformer\_medium & cuda & \textbf{183.7x} \\
      \midrule
      dygformer\_large & Python & 1.0x \\
      dygformer\_large & cuda & \textbf{175.9x} \\
      \bottomrule
    \end{tabular}
    \end{table}
    {\small На синтетических бенчмарках; выигрыш растёт с числом соседей $K$}
  \end{columns}
\end{frame}
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \begin{itemize}
      \item \textbf{pytest} + 47 тестовых файлов
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \begin{itemize}
      \item Объяснить, почему структурные методы превосходят GNN: анализ структуры графа, роль «хабов», повторные взаимодействия
